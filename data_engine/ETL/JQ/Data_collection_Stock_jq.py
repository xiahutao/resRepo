...
for idx, row in securities.iterrows():
    ...
    row_dict['symbol'] = idx
    row_dict['ASSET_TYPE']= setting.ASSETTYPE_STOCK
    collection.update_one(filter={'symbol':row_dict['symbol']},update={'$set':row_dict},upsert=True)


#+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
stocks = DataFactory._get_instruments(asset_type=setting.ASSETTYPE_STOCK)
stocks_jq = DataFactory._get_instruments(asset_type=setting.ASSETTYPE_STOCK,collection='instruments_jq')
...
for idx, row in stocks_jq.iterrows():
    ...
    collection.update_one(filter={'symbol':row_dict['symbol']},update={'$set':{
        ...
        'ASSET_TYPE': setting.ASSETTYPE_STOCK
    }})
